...
dfr_cols <- function(...) {
  ...
  if (any(!(cols %in% names(dfr_colors))))
    warning(paste0("Invalid color names: ", paste(paste0("\"", cols[!(cols %in% names(dfr_colors))], "\""), collapse=", ")))
...
dfr_palettes <- list(
  ...
  `navy` = dfr_cols("dark_navy", "omd_navy", "accent1", "accent2", "accent3", "aqua")
)
...
#'
scale_color_dfr <- function(palette = "main", discrete = TRUE, reverse = FALSE, ...) {
...
#'
scale_fill_dfr <- function(palette = "main", discrete = TRUE, reverse = FALSE, ...) {
  pal <- dfr_pal(palette = palette, reverse = reverse)
